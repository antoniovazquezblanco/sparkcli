...
static void _render_tree(const scli_tree_node_t *node, const char *prefix, int is_last, const _tree_level_style_t *style) {
  ...
  for (size_t i = 0; i < node->child_count; i++) {
    ...
    const char *extra =
        is_last ? style->space : style->vertical;
...
bool scli_tree_add_child(scli_tree_node_t *parent,
                         scli_tree_node_t *child)
{
  if (!parent || !child)
    return false;
